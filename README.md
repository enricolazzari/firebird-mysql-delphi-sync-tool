...
2.  **Delphi Environment:**
    ...
    * On the form, manually adjust all components listed in the `type` section of the code in the file (e.g., `TFDConnection`, `TIBDatabase`, `TButton`, `TProgressBar`, etc.). (**IMPORTANT**: Information has to be the exact same in the INI file)
