...
var
  ...
  i, TotalRegistros, RegistrosNoLote, BatchSize: Integer;
  Campo, FieldList, ParamList, UpdateList, CacheKey: string;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if QMySQL.ParamByName(Campo).DataType = ftUnknown then
          QMySQL.ParamByName(Campo).DataType := FBTypeToDelphiType(fbType);
      end;

      QMySQL.Params.ArraySize := BatchSize;
      RegistrosNoLote := 0;

      while (RegistrosNoLote < BatchSize) and (not qFirebird.Eof) do
      begin
        for i := 0 to ListaCampos.Count - 1 do
        begin
          Campo := ListaCampos[i];
          if not qFirebird.FieldByName(Campo).IsNull then
            QMySQL.ParamByName(Campo).Values[RegistrosNoLote] := qFirebird.FieldByName(Campo).Value
          else
      ...
      if RegistrosNoLote > 0 then
        QMySQL.Execute(RegistrosNoLote);

      FDConnection1.Commit;
      if not qFirebird.Eof then
        FDConnection1.StartTransaction;
    end;
